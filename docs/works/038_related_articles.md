...
- **参照記事の抽出**: 本文（`entry.body`）の生Markdownを `BLOG_LINK_PATTERN` で走査し、slug（= `post.id`）で記事を引く。相対パス `/blog/YYYY/MM/slug/` と旧ドメインの絶対URL `https://skawashima.com/blog/...` の両方が同じパターンで拾える
- **画像パスの除外**: `/blog/2020/05/dont-store-to-dos/example.png` のような記事フォルダ内画像を参照と誤検出しないよう、末尾に先読み `(?=[)\s"'#?]|$)` を置いてスラッシュの後ろにセグメントが続くケースを弾いた
- **外部ホストの除外（レビュー指摘対応）**: 当初のパターンはホストを検証しておらず、`https://example.com/blog/2024/01/local-post/` のような外部サイトの同形式URLも拾ってしまっていた。slugがローカル記事のidと衝突すると、参照していない記事を参照扱いする。先頭に境界＋ホストの並び `(?:^|[\s("'])(?:https?:\/\/(?:www\.)?skawashima\.com)?` を足して、相対パスと旧ドメインだけに限定した。あわせて `m` フラグを付け、末尾先読みの `$` が文字列末ではなく行末を意味するようにした（行末のリンクも拾えるようになる）
...
外部ホスト除外の修正後も、`pnpm build` の生成HTMLは上表とまったく同じ出力だった（既存の挙動を変えずに誤検出だけを塞げている）。あわせて `node` で新旧パターンを突き合わせ、単体8ケース全通過・実コーパス32記事で抽出結果の差分ゼロを確認した。

| 入力 | 修正前 | 修正後 |
| --- | --- | --- |
| `[a](/blog/2018/05/spajam2018-first-hackathon/)` | 検出 | 検出 |
| `https://skawashima.com/blog/2020/05/dont-store-to-dos/` | 検出 | 検出 |
| `https://example.com/blog/2024/01/local-post/` | **誤検出** | 検出しない |
| `![i](/blog/2020/05/dont-store-to-dos/example.png)` | 検出しない | 検出しない |
| `![i](/blog/2018/05/spajam2018-first-hackathon/優秀賞.jpg)` | 検出しない | 検出しない |

## レビュー対応

CodeRabbit（インライン、🟡 Minor）と Claude Code Review が独立に同じ箇所へ収束した。どちらも `src/lib/blog.ts` の `BLOG_LINK_PATTERN` がホストを検証していない点を指摘していたため対応した（上記「外部ホストの除外」）。

CodeRabbit が併せて求めていた回帰テストの追加は見送った。このリポジトリにはテスト基盤が無く（`package.json` に test スクリプトが無い）、その導入は本PRのスコープを超えるため。代わりに既存の方法どおり `pnpm build` の生成HTMLで検証し、加えて `node` でのパターン突き合わせを行った。

## 次のステップ

- 記事数が増えてカテゴリ一致だけの関連が目立つようなら、スコアの重み付けや「タグ一致必須」への変更を検討する
- テスト基盤（vitest 等）の導入。`BLOG_LINK_PATTERN` のようなロジックは本来ユニットテストで守りたいが、現状は毎回 `pnpm build` の出力確認と使い捨てスクリプトに頼っている
